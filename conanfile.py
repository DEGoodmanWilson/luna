...
class LunaConan(ConanFile):
    ...
    def source(self):
        self.run("git clone https://github.com/DEGoodmanWilson/luna.git --branch v%s" % (self.version))
    ...
    def build(self):
        ...
        build_luna_tests = "-DBUILD_LUNA_TESTS=ON" if self.options.build_luna_tests else ""
        build_luna_coverage = "-DBUILD_LUNA_COVERAGE=ON" if self.options.build_luna_coverage else ""
        build_luna_examples = "-DBUILD_LUNA_EXAMPLES=ON" if self.options.build_luna_examples else ""
